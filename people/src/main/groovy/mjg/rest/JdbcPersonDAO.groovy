...
class JdbcPersonDAO implements PersonDAO {
    static Sql db = Sql.newInstance(url:'jdbc:h2:db', driver:'org.h2.Driver')
    ...
    List<Person> findAll() {
        ...
        db.rows(txt).collect { row -> new Person(id:row.id, first:row.first, last:row.last) }
    }

    Person findById(long id) {
        String txt = 'select * from people where id=?'
        def row = db.firstRow(txt, [id])
	...
	List<Person> findByLastName(String name) {
        ...
        customers += db.rows(txt, "%$name%".toLowerCase()).collect { row ->
            new Person(id:row.id, first:row.first, last:row.last) }
        customers += db.rows(txt, "%$name%".toUpperCase()).collect { row ->
            new Person(id:row.id, first:row.first, last:row.last) }
        customers
    }
    ...
    Person create(Person p) {
        ...
        def keys = db.executeInsert txt, [null, p.first, p.last]
        p.id = keys[0][0]
        return p
    }
    ...
    Person update(Person p) {
        ...
        db.execute txt, [p.first, p.last, p.id]
        return p
    }

    boolean delete(long id) {
        String txt = 'delete from people where id=?'
        int uc = db.executeUpdate txt, [id]
        return uc == 1
    }
}
